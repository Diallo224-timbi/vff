...
<main class="main">
    ...
    <div class="quick-actions">
        @php
            // Routes disponibles avec fallback
            $routes = [
                'annuaire.index' => 'Annuaire',
                'structures.map' => 'Cartographie',
                'events.index' => 'Agenda',
                'forum.index' => 'Forum',
        ...
        @foreach($routes as $routeName => $title)
            @if(Route::has($routeName))
                <a href="{{ route($routeName) }}" class="qa-card">
                    <div class="qa-icon qa-icon-{{ $loop->iteration % 2 == 0 ? 'blue' : 'green' }}">
                        <i class="bx bx-{{ $loop->iteration == 1 ? 'folder' : ($loop->iteration == 2 ? 'map' : ($loop->iteration == 3 ? 'calendar' : ($loop->iteration == 4 ? 'message' : 'book'))) }}"></i>
                    </div>
                    <div>
                        <strong>{{ $title }}</strong>
                        <p>{{ $loop->iteration == 1 ? 'Structures, membres, carte interactive' : ($loop->iteration == 2 ? 'Visualisation géographique des acteurs' : ($loop->iteration == 3 ? 'Événements, 8 mars, 25 novembre' : ($loop->iteration == 4 ? 'Échanges entre partenaires' : 'Espace documentaire'))) }}</p>
                    </div>
                </a>
            @endif
        @endforeach
    ...
    <div class="dynamic-section">
        ...
        <div class="affiche-widget">
            ...
            @php
                $documents = [
                    'img/Affiche-ALPES-MARITIMES.pdf' => 'Télécharger l\'affiche (PDF)',
                    'img/SDLVFF 2024-27_VO.odt' => 'Télécharger le schéma (ODT)',
                    'doc/Programmation_GT_Schéma_Avril_2026.pdf' => 'Télécharger la Programmation GT Schéma Avril 2026 (PDF)',
                    'doc/Ville_Nice_VO_2026 _GUIDE_VICTIMES_CONJUGALES_SEXISTES_SEXUELLE.pdf' => 'Télécharger le guide de la Ville de Nice (PDF)',
                    'doc/CARF_RESEAU_RIVIERA_FRANCAISE_guide_victimes.pdf' => 'Télécharger le guide du CARF Réseau (PDF)',
                    'doc/Flyer_Parcours _de _Femmes _2026.pdf' => 'Télécharger le flyer de parcours de femmes 2026 (PDF)'
                ];
            @endphp
            
            @foreach($documents as $path => $label)
                @if(file_exists(public_path($path)))
                <a href="{{ asset($path) }}" download class="btn-dl">
                    <i class="bx bx-download"></i> {{ $label }}
                </a>
...
<script>
    // Optimisation du chargement - éviter les blocages
    ...
    function initPage() {
        // Animation au scroll avec debounce pour performance
        let timeout;
        ...
        // Gestionnaire d'erreur global pour les ressources
        window.addEventListener('error', function(e) {
            console.warn('Ressource non chargée:', e.target.src || e.target.href);
...
<!-- Fallback si Boxicons ne charge pas -->
<noscript>
    <style>
        [class^="bx-"], [class*=" bx-"] {
