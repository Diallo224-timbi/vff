        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" id="members-container">
            @foreach($membres as $membre)
                <div class="member-card bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 overflow-hidden group">
                    <div class="h-1 bg-gradient-to-r from-[#4a8599] to-[#255156]"></div>
                    <div class="p-6">
                        <h2 class="text-xl font-bold mb-3 text-gray-800 group-hover:text-blue-600 transition-colors">{{ $membre->name }} {{ $membre->prenom }}</h2>
                        <div class="space-y-2">
                            <p class="text-gray-600 flex items-start">
                                ...
                                <span><strong class="text-gray-700">Email:</strong> <span class="member-email">{{ $membre->email }}</span></span>
                            </p>
                            <p class="text-gray-600 flex items-start">
                                <svg class="w-5 h-5 mr-2 text-blue-500 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <p class="text-gray-600 flex items-start">
                                ...
                                <span><strong class="text-gray-700">Téléphone:</strong> <span class="member-phone">{{ $membre->phone }}</span></span>
                            </p>
                        </div>
                    </div>
                </div>
...
<script>
    // Script pour gérer la recherche dynamique des membres
    ...
    function filterMembers() {
        ...
        const structureValue = structureFilter.value;
        let visibleCount = 0;
        ...
        memberCards.forEach(function(card) {
            ...
            // Vérifie si la structure correspond au filtre
            const matchesStructure = structureValue === '' || 
                                    structure.includes(structureValue);
    ...
    // Événements avec debounce pour la recherche
    let timeout;
    ...
    // Exécute le filtre au chargement de la page
    filterMembers();
</script>
